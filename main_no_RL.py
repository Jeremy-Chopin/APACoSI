...
from pyqap.utils import load_knowledge, load_file, get_diagonal_size, create_images_from_ids
...
# Paths and Images
pr_mask = np.load(os.path.join('tutorial','test_cnn_output.npy'))
image_cnn = np.argmax(pr_mask, axis=2)

gt_mask, affine = load_file(os.path.join('tutorial','test_gt.png'))
intensity_input, _ = load_file(os.path.join('tutorial','test_rgb.png'))

KNOWLEDGES_DIR = os.path.join('tutorial','knowledges')
